...
(source: AsyncComponentLoader<T> | AsyncComponentOptions<T>): T {
  ...
  const {
    ...
    delay = 200,
    timeout, // undefined =never time out
    suspensible = true,
    onError: userOnError
  } = source
          ...
          .catch(err => {
            ...
            if (userOnError) {
              return new Promise((resolve, reject) => {
                const useRetry = () => resolve(retry())
                const userFail = () => reject(err)
                userOnError(err, useRetry, userFail, retries + 1)
              })
            } else {
              throw err
            }
    ...
    setup() {
      ...
      const onError = (err: Error) => {
        ...
        handleError(
          ...
          !errorComponent /* 如果用户提供错误组件，不要在dev中抛出。 */
        )
      }
      ...
      if (
        ...
        return load()
          ...
          .catch(err => {
            onError(err)
            return () =>
              errorComponent ? createVNode(errorComponent as ConcreteComponent, {
                error: err
              }) : null
          })
      }
