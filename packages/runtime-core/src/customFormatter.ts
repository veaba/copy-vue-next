...
export function initCustomFormatter() {
  ...
  const formatter = {
    header(obj: unknown) {
      // TODO: 同时格式化ComponentPublicInstance&ctx.插槽/属性在设置中
      if (!isObject(obj)) {
        return null
      }
  ...
  function createInstanceBlock(type: string, target: any) {
    ...
    return [
      'div',
      { style: 'inline-height:1.25em;margin-bottom:0.6em' },
      [
        'div',
        {
          style: 'color:#476582'
